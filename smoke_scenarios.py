...
def run() -> dict:
    ...
    with TestClient(app) as client:
        ...
        # Scenario 4: compare channels -> verify winner/ranking/actions structure
        compare_payload = {
            "my_channel_name": "Мой бренд",
            "my_channel_url": "t.me/s/my_brand_channel",
            "competitors": [
                {"name": "Конкурент A", "url": "t.me/s/competitor_a"},
                {"name": "Конкурент B", "url": "https://example.com/competitor-b"},
            ],
            "posts_limit": 12,
        }

        compare_response = client.post("/compare_channels", json=compare_payload)
        assert_true(compare_response.status_code == 200, "compare_channels must return 200")
        compare_body = compare_response.json()

        ranking = compare_body.get("ranking", [])
        insights = compare_body.get("insights", [])
        actions = compare_body.get("actions", [])
        winner = compare_body.get("winner", {})
        details = compare_body.get("channel_details", [])

        assert_true(isinstance(ranking, list) and len(ranking) >= 2, "ranking must contain my channel and competitors")
        assert_true(bool(winner.get("name")), "winner must contain channel name")
        assert_true(bool(winner.get("reason")), "winner must contain reason")
        assert_true(isinstance(insights, list) and len(insights) > 0, "insights must be non-empty")
        assert_true(isinstance(actions, list) and len(actions) > 0, "actions must be non-empty")
        assert_true(isinstance(details, list) and len(details) == len(ranking), "channel_details must align with ranking")
